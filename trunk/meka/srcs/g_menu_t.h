...
#define gui_menu_check(menu_id,n_entry)         do { menus[(menu_id)]->entry[(n_entry)]->flags |= MENU_ITEM_FLAG_CHECKED; } while (0)
#define gui_menu_un_check_one(menu_id,n_entry)  do { menus[(menu_id)]->entry[(n_entry)]->flags &= ~MENU_ITEM_FLAG_CHECKED; } while (0)
